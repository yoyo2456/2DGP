...
def update(frame_time):
    ...
    #Player, AI Collide
    if collide(player,ai_1):
          if (player.state == player.LEFT_MOVE or player.state == player.STANDING)  and (player.bf_state != player.DOWN_MOVE or player.bf_state != player.UP_MOVE): #Collison 보정
               if player.x > ai_1.x:
                   player.leftCollide_Player()
                   ai_1.state = ai_1.STANDING
                   player.bf_state = player.LEFT_MOVE

          if (player.state == player.RIGHT_MOVE or player.state == player.STANDING) and (player.bf_state != player.DOWN_MOVE or player.bf_state != player.UP_MOVE): #Collison 보정
               if player.x < ai_1.x:
                   player.rightCollide_Player()
                   ai_1.state = ai_1.STANDING
                   player.bf_state = player.RIGHT_MOVE

          if (player.state == player.UP_MOVE or player.state == player.STANDING) and (player.bf_state != player.LEFT_MOVE or player.bf_state != player.RIGHT_MOVE): #Collison 보정
               if player.y < ai_1.y:
                   player.upCollide_Player()

          if (player.state == player.DOWN_MOVE or player.state == player.STANDING) and (player.bf_state != player.LEFT_MOVE or player.bf_state != player.RIGHT_MOVE): #Collison 보정
               if player.y > ai_1.y:
                   player.downCollide_Player()



          if player.state == player.LEFT_ATTACK and ai_1.state == ai_1.STANDING:
               if player.x > ai_1.x:
                   ai_1.hp -= 0.7
               if ai_1.hp <= 0:
                   ai_1.switch = False

          if player.state == player.RIGHT_ATTACK and ai_1.state == ai_1.STANDING:
               if player.x < ai_1.x:
                  ai_1.hp -= 0.7
               if ai_1.hp <= 0:
                   ai_1.switch = False

          if player.state == player.UP_ATTACK and ai_1.state == ai_1.STANDING:
               if player.y < ai_1.y:
                   ai_1.hp -= 0.7
               if ai_1.hp <= 0:
                   ai_1.switch = False

          if player.state == player.DOWN_ATTACK and ai_1.state == ai_1.STANDING:
              if player.y > ai_1.y:
                  ai_1.hp -= 0.7
              if ai_1.hp <= 0:
                  ai_1.switch = False
    ...
    for in_tile in in_tiles:
       if collide(player,in_tile):
           if player.state == player.LEFT_MOVE :
               if player.x > in_tile.x:
                   player.leftCollide_Player()
                   player.bf_state = player.LEFT_MOVE

           if player.state == player.RIGHT_MOVE :
               if player.x < in_tile.x:
                   player.rightCollide_Player()
                   player.bf_state = player.RIGHT_MOVE

           if player.state == player.UP_MOVE :
               if player.y < in_tile.y:
                   player.upCollide_Player()
                   player.bf_state = player.UP_MOVE

           if player.state == player.DOWN_MOVE:
               if player.y > in_tile.y:
                   player.downCollide_Player()
           ...
           if player.state == player.LEFT_ATTACK:
               if player.x > in_tile.x:
                   player.leftCollide_Player()
                   player.bf_state = player.LEFT_ATTACK

           if player.state == player.RIGHT_ATTACK:
               if player.x < in_tile.x:
                   player.rightCollide_Player()
                   player.bf_state = player.RIGHT_ATTACK

           if player.state == player.UP_ATTACK:
               if player.y < in_tile.y:
                   player.upCollide_Player()
                   player.bf_state = player.UP_ATTACK

           if player.state == player.DOWN_ATTACK:
               if player.y > in_tile.y:
                   player.downCollide_Player()
                   player.bf_state = player.DOWN_ATTACK
...
def draw(frame_time):
    ...
    #font.draw(320,590, 'Dir : %d' % (player.direction),(0,255,0))

   # font.draw(320,550, '[Player X : %d][Player Y : %d]' % (player.x, player.y),(0,255,0))
    #font.draw(320,520, '[AI X : %d][AI Y : %d]' % (ai_1.x,ai_1.y),(255,255,0))
    font.draw(320,490, '[AI HP : %d]' % (ai_1.hp),(255,255,0))
